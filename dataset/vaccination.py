...
COL_DATE = 'data_de_vacinacao'
COL_FIRST_DOSE = 'dose_1'
COL_SECOND_DOSE = 'dose_2'
COL_UNIQUE_DOSE = 'dose_unica'
COL_REINFORCEMENT_DOSE = 'dose_de_reforco'
COL_TOTAL = 'total'

COL_DATE_ON_CASE = 'data_notificacao'
COL_VACCINATION_PERCENTAGE = 'populacao_vacinada'
class DatasetVaccination(DatasetBase):
    ...
    def append_vaccination_percentage(self, cases):
        cases[COL_VACCINATION_PERCENTAGE] = 0.0
        for date, percentage in self.percentage_on_day.items():
            cases.loc[cases[COL_DATE_ON_CASE] == date, COL_VACCINATION_PERCENTAGE] = percentage
        return cases
        
    def remove_extra_columns(self):
        self.df.dropna(axis=1, how='all', inplace=True)
        self.df.drop([COL_FIRST_DOSE, COL_REINFORCEMENT_DOSE, COL_TOTAL], axis=1, inplace=True)
    ...
    def count(self):
        ...
        for index, row in self.df.iterrows():
            day_count = row[COL_SECOND_DOSE] + row[COL_UNIQUE_DOSE]
            cumulative_count += day_count
            # Get the string date on COL_DATE and format it to YYYY-MM-DD
            date = row[COL_DATE]
            date = date.split('/')
